...
lam_max = 2.5
step = .1  # resolution
length = int(np.ceil((lam_max - lam_min) / step))  # number of steps
...
for i, lam in enumerate(lam_vek):
    ...
    X_exhaust = q3.X  # [mol/mol] molfractions of gas and air mixture
    exhaust.TPX = T, p, X_exhaust
    exhaust.equilibrate("HP")  # equilibrium with constant pressure and enthalpie
